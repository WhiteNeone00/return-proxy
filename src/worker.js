...
const cloudflareHostState = async (hostname, env) => {
  if (!env?.CF_API_TOKEN) return 'secret-missing';
  const headers = { authorization: `Bearer ${env.CF_API_TOKEN}` };
  try {
    const zoneResponse = await fetch('https://api.cloudflare.com/client/v4/zones?name=insideproxy.me&per_page=1', { headers });
    if (!zoneResponse.ok) return `zone-api-${zoneResponse.status}`;
    const zonePayload = await zoneResponse.json();
    const zoneId = zonePayload.result?.[0]?.id;
    if (!zoneId) return 'zone-not-found';

    const dnsResponse = await fetch(`https://api.cloudflare.com/client/v4/zones/${zoneId}/dns_records?name=${encodeURIComponent(hostname)}&per_page=100`, { headers });
    if (!dnsResponse.ok) return `dns-api-${dnsResponse.status}`;
    const dnsPayload = await dnsResponse.json();
    if (dnsPayload.result?.some((record) => record.name === hostname)) return 'present';

    // An exact DNS miss is enough to classify an unregistered hostname.
    // The wildcard record is intentionally ignored because its name is not exact.
    if (Array.isArray(dnsPayload.result)) return 'missing';
    return 'dns-invalid-response';
  } catch {
...
export default {
  async fetch(request, env) {
      ...
      try {
        ...
        if (isWorkerHop) {
          ...
          if (env?.ORIGIN_URL) {
            ...
            upstreamRequest = new Request(originUrl, {
    ...
    const genericStatus = requestedCode >= 400 && requestedCode <= 599 ? {
      code: requestedCode,
      title: requestedCode === 404 ? 'Page not found' : requestedCode === 502 ? 'Site temporarily unavailable' : requestedCode === 503 ? 'Service unavailable' : requestedCode === 504 ? 'Request timed out' : `Request returned ${requestedCode}`,
      description: requestedCode === 404 ? 'The page you requested does not exist, or it may have moved somewhere else.' : requestedCode >= 500 ? 'The service is temporarily unavailable. Please try again in a few moments.' : 'The request could not be completed. Please check the request and try again.',
    ...
    const status = genericStatus || (notFound ? {
      code: 404,
      title: 'Page not found',
      description: 'The page you requested does not exist, or it may have moved somewhere else.',
      icon: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="9"/><path d="m9 9 6 6m0-6-6 6"/></svg>'
    } : originDown ? {
      code: 502,
      title: 'Site temporarily unavailable',
      description: 'This domain is active, but the site behind it is not responding right now. Please try again shortly.',
      icon: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M10.29 3.86 1.82 18a2 2 0 0 0 1.71 3h16.94a2 2 0 0 0 1.71-3L13.71 3.86a2 2 0 0 0-3.42 0z"/><line x1="12" y1="9" x2="12" y2="13"/><line x1="12" y1="17" x2="12.01" y2="17"/></svg>'
    } : serviceUnavailable ? {
      code: 503,
      title: 'Service unavailable',
      description: 'Return Proxy is online, but this service is temporarily unavailable. Please try again in a few moments.',
      icon: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M10.29 3.86 1.82 18a2 2 0 0 0 1.71 3h16.94a2 2 0 0 0 1.71-3L13.71 3.86a2 2 0 0 0-3.42 0z"/><line x1="12" y1="9" x2="12" y2="13"/><line x1="12" y1="17" x2="12.01" y2="17"/></svg>'
    } : {
      code: 504,
      title: 'Request timed out',
      description: 'Return Proxy is online, but the upstream service took too long to respond. Please try again in a few moments.',
      icon: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M10.29 3.86 1.82 18a2 2 0 0 0 1.71 3h16.94a2 2 0 0 0 1.71-3L13.71 3.86a2 2 0 0 0-3.42 0z"/><line x1="12" y1="9" x2="12" y2="13"/><line x1="12" y1="17" x2="12.01" y2="17"/></svg>'
    });
